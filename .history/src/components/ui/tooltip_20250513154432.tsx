...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  return (
    <TooltipContext.Provider value={contextValue}>
      {disabled ? children : (
        <TooltipTrigger delayDuration={delayDuration} hideDelay={hideDelay} asChild={asChild}>
          {children}
        </TooltipTrigger>
      )}
...
const TooltipTrigger = React.forwardRef<HTMLDivElement, TooltipTriggerProps>(
  ({ children, delayDuration, hideDelay, asChild = false }, ref) => {
...
const TooltipContentDisplay = () => {
  ...
  return (
    <>
      <div className="fixed z-50 left-0 top-0 h-0">
        <AnimatePresence>
          <motion.div
            ref={contentRef}
            style={{ 
          ...
          >
            {!config.hideArrow && ( 
              <div 
                className={cn("w-2 h-2 absolute", getVariantClasses())} 
                style={getArrowStyle()}
              />
            )}
            {content}
