...
const Tabs = React.forwardRef<HTMLDivElement, TabsProps>(
  ({ className, defaultValue, value, onValueChange, children, ...props }, ref) => {
    ...
    const tabTriggerRefs = React.useRef<Map<string, HTMLButtonElement | null>>(new Map());
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        "relative inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-primary ",
        className
      )}
      {...props}
    >
      <div 
        className="tabs-bg-indicator bg-gradient-tabs animate-move-bg absolute rounded-sm transition-all duration-300 ease-spring" 
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isActive && "text-white dark:text-black", // <-- Add this line
        className
      )}
...
>(({ className, value, ...props }, ref) => {
  ...
  return (
    <div
      ...
      data-value={value}
      className={cn(
        "mt-2 ring-offset-background focus-visible:outline-none overflow-scroll focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
        className
      )}
