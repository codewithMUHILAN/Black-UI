...
export const DynamicNavigation = ({
  ...
  const highlightStyles = {
    backgroundColor:
      theme === "custom" && highlightColor ? highlightColor : undefined,
  };
  ...
  return (
    <nav
      ref={navRef}
      className={cn(
        `relative rounded-full px-2 backdrop-blur-md border 
    ...
    >
      ...
      <ul className="flex justify-between items-center gap-1 py-2 relative z-10">
        {links.map((link) => (
          <li
            key={link.id}
          ...
          >
            <a
              href={link.href}
              className={cn(
                `flex gap-2 items-center justify-center h-8 md:h-10 text-xs md:text-sm 
            ...
            >
              {link.icon && (
                <span className="text-current text-xs md:text-sm ">
                  {link.icon}
                </span>
              )}
              <span
