...
export function SidebarContent({ className, children, ...props }: SidebarContentProps) {
  return (
    <div className={cn("flex-1 overflow-hidden h-[calc(100vh-4rem)]", className)} {...props}>
      <div className="h-full pb-12 overflow-auto no-scrollbar">
        {children}
      </div>
    </div>
  );
...
export function SidebarMenu({ className, children, ...props }: SidebarMenuProps) {
  ...
  return (
    <div ref={menuRef} className={cn("relative", className)} {...props}>
      <div className="tabs-bg-indicator opacity-0 absolute transition-all duration-300 ease-in-out rounded-md bg-primary/10 border border-primary/20" />
      {children}
    </div>
  );
}
